...
import { loginWithCredentials, registerCustomer, registerTechnician, resetPassword } from './authStore';
import './AuthModal.css';

export default function AuthModal({ isOpen, onClose, initialMode = 'login', initialRole = 'customer', onAuthSuccess }) {
  const [mode, setMode] = useState(initialMode); // 'login' | 'register' | 'forgot' | 'tech_register' | 'tech_status'
  ...
  const handleGoogleSignIn = () => {
    // Simulated frictionless Google OAuth Sign-In
    const googleUser = {
      id: `GOOG-${Math.floor(1000 + Math.random() * 9000)}`,
      role: role === 'technician' ? 'technician' : 'customer',
      name: 'Sherhan Abid',
      email: 'sherhanabid123@gmail.com',
      phone: '9847012345',
      city: 'Kannur',
      status: 'approved'
    };

    localStorage.setItem('homefix_current_user', JSON.stringify(googleUser));
